<template>
  <div
    class="shadow-md flex justify-between flex-col relative h-full p-8 overflow-auto bg-badge rounded-md hover:-translate-y-0.5 hover:scale-105 hover:cursor-zoom-in trans"
  >
    <div>
      <div class="flex items-center justify-between w-full">
        <span class="text-4xl text-orange">
          <Icons title="folder" />
        </span>
        ...
        <div class="flex items-center gap-3.5" v-if="project.isLive">
          <a
            ...
            :key="index"
            class="text-2xl text-secondary hover:text-orange transition-all duration-300 cursor-pointer"
            target="_blank"
            :href="link.url"
            :title="link.title"
          >
    ...
    <footer class="flex gap-2 mt-5 flex-wrap">
      <span
        ...
        :key="tag"
        class="rounded-md text-xs leading-5 px-3 font-normal text-white bg-opacity-90 bg-[#2f293ae8] text-center border border-secondary !py-1"
      >
        {{ tag }}
      </span>
    </footer>
